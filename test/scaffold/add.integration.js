...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/veriumcore-node.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update veriumcore-node.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'veriumcore-lib': '^v0.13.7',
          'veriumcore-node': '^v0.2.0'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/veriumcore-node.json');
