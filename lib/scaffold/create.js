...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'veriumcore-lib': '^' + bitcore.version,
    'veriumcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/veriumcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the veriumcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the veriumcore-node directory and configuration
