...
var bitcore = require('veriumcore-lib');
var $ = bitcore.util.preconditions;
var _ = bitcore.deps._;
var utils = require('../utils');

/**
 * Will remove a service from veriumcore-node.json
...
/**
 * Will remove the Node.js service and from the veriumcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The veriumcore-node configuration directory
...
function remove(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'veriumcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a veriumcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to veriumcore-node.json
